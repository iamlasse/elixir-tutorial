defmodule TutorialWeb.WalletControllerTest do
  ...
  describe "index" do
    test "lists all wallets", %{conn: conn} do
      conn = get conn, cms_wallet_path(conn, :index)
  ...
  describe "new wallet" do
    test "renders form", %{conn: conn} do
      conn = get conn, cms_wallet_path(conn, :new)
  ...
  describe "create wallet" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, cms_wallet_path(conn, :create), wallet: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == cms_wallet_path(conn, :show, id)

      conn = get conn, cms_wallet_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Show Wallet"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, cms_wallet_path(conn, :create), wallet: @invalid_attrs
      assert html_response(conn, 200) =~ "New Wallet"
    end
  end
  ...
  describe "edit wallet" do
    ...
    test "renders form for editing chosen wallet", %{conn: conn, wallet: wallet} do
      conn = get conn, cms_wallet_path(conn, :edit, wallet)
  ...
  describe "update wallet" do
    ...
    test "redirects when data is valid", %{conn: conn, wallet: wallet} do
      conn = put conn, cms_wallet_path(conn, :update, wallet), wallet: @update_attrs
      assert redirected_to(conn) == cms_wallet_path(conn, :show, wallet)

      conn = get conn, cms_wallet_path(conn, :show, wallet)
      assert html_response(conn, 200) =~ "some updated type"
    end

    test "renders errors when data is invalid", %{conn: conn, wallet: wallet} do
      conn = put conn, cms_wallet_path(conn, :update, wallet), wallet: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Wallet"
    end
  end
  ...
  describe "delete wallet" do
    ...
    test "deletes chosen wallet", %{conn: conn, wallet: wallet} do
      conn = delete conn, cms_wallet_path(conn, :delete, wallet)
      assert redirected_to(conn) == cms_wallet_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, cms_wallet_path(conn, :show, wallet)
      end
    end
  end
